...
- (void)setCloseButtonType:(ButtonPositionType)closeButtonType {
    ...
    if (closeButtonType == ButtonPositionTypeNone) {
        ...
        if(closeButtonType == ButtonPositionTypeRight){
            closeFrame.origin.x = round(CGRectGetWidth(self.containerView.frame) - kDefaultCloseButtonPadding - CGRectGetWidth(closeFrame)/2)-13;
            closeFrame.origin.y = 10;
        } else {
            closeFrame.origin.x = 13;
            closeFrame.origin.y = 10;
        }
...
- (void)showWithPresentView:(UIView *)presentView animated:(BOOL)animated {
  ...
  //  closeButton.clipsToBounds = true;
   // closeButton.clipsToBounds = true;
   // [closeButton.layer setCornerRadius:CGRectGetWidth(closeButton.bounds)*0.5];
...
- (instancetype)init {
    ...
    if (self) {
        ...
        [self setBackgroundImage:[UIImage imageNamed:@"red_packge_close2"] forState:UIControlStateNormal];
